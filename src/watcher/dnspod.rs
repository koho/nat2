...
impl DnsPod {
    ...
    async fn create_record(&self, record: Record) -> Result<u64> {
        ...
        let record_id = resp.response.record_id.ok_or(anyhow!("record id not found in response"))?;
        debug!(request_id=resp.response.common.request_id, record_id, name=self.name(), "create record succeed");
        Ok(record_id)
    }
    ...
    async fn update_record(&self, record_id: u64, record: Record) -> Result<u64> {
        ...
        resp.response.common.error.map(|v| Err::<(), anyhow::Error>(anyhow!("{}: {}", v.code, v.message))).transpose()?;
        let record_id = resp.response.record_id.ok_or(anyhow!("record id not found in response"))?;
        debug!(request_id=resp.response.common.request_id, record_id, name=self.name(), "update record succeed");
        Ok(record_id)
    }
}
